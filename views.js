...
exports.dashboardPage = (user, accounts, platforms) => {
  ...
  const cellHtml = (p, key) => {
    const acc = get(p);
    if (!acc) return `<td class="placeholder" id="cell-${p}-${key}">sample</td>`;
    const v = acc[key];
    if (key === 'revenue') {
      return `<td id="cell-${p}-${key}">${v === null || v === undefined ? '<span class="na">N/A</span>' : '$ ' + Number(v).toLocaleString()}</td>`;
    }
    return `<td id="cell-${p}-${key}">${fmt(v)}</td>`;
  };

  const rows = rowLabels.map(([key, label]) => `
    <tr>
      <td class="row-label">${label}</td>
      ${cols.map(p => cellHtml(p, key)).join('')}
    </tr>`).join('');
  ...
  return layout('Dashboard', {
    ...
    <p class="muted" id="sync-status">Live · refreshes every 15s</p>
    <table>
      <tr>
    ...
    <script>
      const ROW_KEYS = ${JSON.stringify(rowLabels.map(r => r[0]))};
      const PLATFORM_KEYS = ${JSON.stringify(cols)};
      ...
      async function refreshDashboard() {
        try {
          ...
          const byPlatform = {};
          data.accounts.forEach(a => { byPlatform[a.platform] = a; });

          PLATFORM_KEYS.forEach(p => {
            const acc = byPlatform[p];
            if (!acc) return;
            ROW_KEYS.forEach(key => {
              const cell = document.getElementById('cell-' + p + '-' + key);
              if (!cell) return;
              const v = acc[key];
              if (key === 'revenue') {
                cell.innerHTML = (v === null || v === undefined) ? '<span class="na">N/A</span>' : '$ ' + Number(v).toLocaleString();
              } else {
                cell.textContent = fmtNum(v) === null ? 'N/A' : fmtNum(v);
              }
          ...
          if (statusEl) {
            const t = new Date(data.serverTime).toLocaleTimeString();
            statusEl.textContent = 'Live · last checked ' + t + ' · refreshes every 15s';
          }
        } catch (e) {
...
exports.connectPage = (user, accounts, platforms) => {
  ...
  const cards = Object.entries(platforms).map(([key, cfg]) => {
    ...
    if (acc && acc.status === 'connected') {
      ...
      const syncBtn = cfg.livePullSupported
        ? `<form method="POST" action="/sync/${key}" style="margin-top:6px;"><button type="submit" class="btn secondary" style="width:auto; padding:4px 10px; font-size:12px; margin-top:0;">Sync now</button></form>`
        : '';
      actionHtml = `<span class="muted">Last synced ${acc.last_synced_at ? new Date(acc.last_synced_at).toLocaleTimeString() : 'just now'}</span>${syncBtn}`;
    } else if (acc && acc.status === 'error') {
      statusHtml = `<span class="status-pill status-pending">Token expired</span>`;
      actionHtml = `<a class="btn secondary" href="/connect/${key}/start">Reconnect ${cfg.label}</a>`;
